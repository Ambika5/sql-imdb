...
Rank () over(order by movie_id desc) as genre_rank
from genre
...
-- CTE: computes the top 3 genres using average ratings>8 condition and heighest movie counts.
-- using the top genres derived from CTE, the directors are found whose movies have an average ratings > 8 and sorted based on number of movies made.
